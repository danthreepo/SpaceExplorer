...
# Initialize start menu, buttons, and sound
func _ready() -> void:
	...
	if $MenuSound.stream and not $MenuSound.playing:
		$MenuSound.play()
	print("Start menu displayed") # Debug
...
func load_high_score() -> void:
	if FileAccess.file_exists("user://high_score.save"):
		...
		print("Loaded high score: ", high_score) # Debug
	else:
		$HighScoreLabel.text = "High Score: 0"
		print("No high score file found") # Debug
...
func save_high_score(score: int) -> void:
	...
	if score > current_high_score:
		...
		file.close()
		print("Saved high score: ", score) # Debug
...
func _on_reset_score_button_pressed() -> void:
	print("Reset score button pressed") # Debug
	if FileAccess.file_exists("user://high_score.save"):
		DirAccess.remove_absolute("user://high_score.save")
		$HighScoreLabel.text = "High Score: 0"
		print("High score reset") # Debug

# Loop background sound
func _on_menu_sound_finished() -> void:
	print("Menu sound finished, replaying sound") # Debug
	if $MenuSound.stream and not $MenuSound.playing:
		$MenuSound.play()
